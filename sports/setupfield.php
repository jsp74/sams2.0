...
<body>
    ...
    <div class="jumbotron">
        <strong class="text-center">
            <h1>Archery Event Preferences</h1>
        </strong>
    </div>
    ...
    <nav class="navbar navbar-inverse">
        ...
        <div class="collapse navbar-collapse" id="myNavbar">
            <ul class="nav navbar-nav">
                ...
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">Master Page <span
                            class="caret"></span></a>
                    <ul class="dropdown-menu">
                        ...
                        <li><a id="find_register" href="./../masterpage/findregistration.php">Find All Registration</a>
                        </li>
                ...
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">Result International Format <span
                            class="caret"></span></a>
                    <ul class="dropdown-menu">
                        <li><a id="extrackfield" href="./../resultpage/exporttrack.html">Export Track &amp; Field</a>
                        </li>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="text-center">
                                    <button type="button" class="btn btn-info" onclick="editTable(0)" id="addRow">Add
                                        Row</button>
                                    <button type="button" class="btn btn-info" onclick="editTable(1)"
                                        id="makeChanges">Make Changes</button>
                                    <button type="button" class="btn btn-info" onclick="saveChanges()"
                                        id="submitChanges">Save Changes</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
function populateTable(data) {
    ...
    for (var x = 1; x < lines.length; x++) {
        ...
        generateTable.innerHTML += "<tbody><tr><td class='text-center' id='event" + x + "'>" + value0 +
            "</td><td class='text-center' id='name" + x + "'>" + value1 + "</td><td class='text-center' id='type" + x +
            "'>" + value2 + "</td><td class='text-center' id='team" + x + "'>" + value3 +
            "</td><td class='text-center' id='date" + x + "'>" + value4 + "</td></tr></tbody>";
    }
}
...
function editTable(x) {
    ...
    if (x == 0) {
        ...
        table.innerHTML += ("<tbody><tr><td id='Event" + size + "'>Event " + size + "</td><td id='name" + size +
            "'></td><td id='type" + size + "'></td><td id='team" + size + "'></td><td id='date" + size +
            "'></td></tr>");
...
function saveChanges() {
    ...
    for (var i = 1; i < size; i++) {
        ...
        //Check
        console.log(i + 1);
        var preEvent = document.getElementById('event' + i);
        var event;
        if (preEvent != null || preEvent != "") {
            event = document.getElementById('name' + i).innerHTML;
            row.push(event);
        } else {
            row.push(" ");
        ...
        var preName = document.getElementById('name' + i).innerHTML;
        var name;
        if (preName != null || preName != "") {
            name = preName.innerHTML;
            row.push(name);
        } else {
            row.push(" ");
        ...
        var preTeam = document.getElementById('team' + i).innerHTML;
        var team;
        if (preTeam != null || preTeam != "") {
            team = preTeam.innerHTML;
            row.push(team);
        } else {
            row.push(" ");
        ...
        row.push(team);
        var preDate = document.getElementById('date' + i);
        var date;
        if (preDate != null) {
            date = preDate.innerHTML;
            row.push(date);
        } else {
    ...
    http.open("POST", "handleField.php", true);
    var params = "data=" + "<?php serialize(" + allRows + ") ?>&sport=field";
